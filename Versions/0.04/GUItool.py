...
class Frames : ##All entry frames !
    def entryframe(conn,root,table):
...
class Table: ##Stores all the Tables that will be used in any GUI file upfront
    def billtable(conn,root): ##This is a Table that stores all the info of a bill before transferring it into the database !
        
        billframe  = LabelFrame(root,text="Bill")

        errorlabel = Label(billframe, text= "") 
        billnumber = int(DBtool.Select.lastID(conn))+1
        print(billnumber)
